...
def l2_loss(model, latent, args, condition, label_idx, latent_size=64):
    ...
    # import pdb; pdb.set_trace() #Ankit
    pred_errors = eval_error_training(
        model,
        scheduler,
        latent,
        all_noise,
...
def eval_error_training(
    ...
    with torch.set_grad_enabled(True):
        for _ in tqdm.trange(
            ...
            required_idx = [
                (j * 10 + torch.div(k, 100, rounding_mode="floor")).item()
...
def load_class_embeddings(model):
    ...
    f = open("/home/soumyajit/Project(-1)/DATASETS/10_worst_liw/find_me.json")
    data = json.load(f)
...
# def load_class_embeddings(model):
#     ## Load the path of embedding manager
#     # f = open("birds_images/val.json")
#     f = open(
#         "/home/soumyajit/ProSpect_Classifier/ProSpect_Classifier/misc/new_train_changed_text.json"
#     )
#     data = json.load(f)
...
#     ## Loading the embedding
#     for j, k in zip(data["labels_idx"], data["label_names"]):
#         print(j, k)

#         model.cpu()
#         model.embedding_manager.load(
#             "/home/soumyajit/ProSpect_Classifier/ProSpect_Classifier/logs/caspian_tern/embeddings_gs-3499.pt"
#         )
#         model = model.to(device)

#         prompts = [f"An image of {k}"]
...
def main():
    ...
    dataloader_params = dict(
        image_size=224, num_aug=100, n_way=5, n_support=0, n_episode=300, n_query=5
    )
    ...
    for task_id, (x, x_ft, y) in enumerate(pbar):
        ...
        try:
            with open(
                "ProSpect_Classifier/error_dict/error_dict_finetune_del.json", "r"
            ) as file:
        ...
        if initial_accuracy <= (correct / total):
            ...
            torch.save(
                class_embeddings,
                f=f"ProSpect_Classifier/logs/class_embeddings_l2+Contrast_{task_id}.pth",
            )
        ...
        with open(
            "ProSpect_Classifier/error_dict/error_dict_finetune_del.json", "w"
        ) as file:
        ...
        with open(filename, "w") as f:
            ...
            f.write(
                f"The accuracy for the model is {correct/total * 100 : .3f} % , Date and Time: {current_time}, on device : {torch.cuda.current_device()}\n"
            )
