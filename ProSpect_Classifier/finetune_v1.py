...
inference_file = "./ProSpect_Classifier/data/CUB'/train/train.json"       ### what inference file we are using
...
def l2_loss(model,latent,args,condition,label_idx,latent_size=64):
    ...
    ts = [int(random.uniform(0,T)) for _ in range(max_n_samples)]
    noise_idxs = range(0,max_n_samples)
    text_embed_idxs = [label_idx for _ in range(max_n_samples)]
    
    # import pdb; pdb.set_trace() #Ankit
    pred_errors = eval_error_training(model, scheduler ,latent, all_noise, ts, noise_idxs,
                                 condition, text_embed_idxs,args.batch_size,args.dtype, args.loss)
    
    loss = torch.mean(pred_errors)
    
    return loss


def eval_error_training(model, scheduler, latent, all_noise, ts, noise_idxs,
    ...
    with torch.set_grad_enabled(True):
        for _ in tqdm.trange(len(ts) // batch_size + int(len(ts) % batch_size != 0), leave=False):
            ...
            required_idx = [(j*10+ torch.div(k,100, rounding_mode='floor')).item() for j,k in zip(text_embed_idxs[idx:idx+batch_size],batch_ts)]       #Ankit
            # required_idx = [(j*10+ (k//100)).item() for j,k in zip(text_embed_idxs[idx:idx+batch_size],batch_ts)]
            text_input = text_embeds[required_idx]
            
            noise_pred = model.apply_model(noised_latent,t_input,text_input)  # shape is (5, 4, 64, 64)
            ...
            pred_errors[idx: idx + len(batch_ts)] = error
            # import pdb; pdb.set_trace() #Ankit
            idx += len(batch_ts)
    return pred_errors
...
def  contrastive_loss(condition,label_idx):
    ...
    for i in range(prospect_stage):
        loss_list = torch.exp(torch.stack([F.cosine_similarity(condition[label_idx*10+i],condition[j*10+i]) for j in range(n_way)] )) 
        loss += torch.sum(loss_list)
        
    # noise_pred = model.apply_model(noised_latent,t_input,text_input)
...
def load_class_embeddings(model):
    ...
    for i,j in zip(data["path"],data["label_idx"]):
        ...
        class_embedding[j] = torch.cat(model.get_learned_conditioning(prompts, prospect_words=prospect_words),dim=0)
...
def main():
    ...
    class_embeddings = torch.load("ProSpect_Classifier/class_embeddings_actual.pth")
    # class_embeddings = {i :torch.randn((10,77,768)).to(device) for i in range(7)} #Ankit
    ...
    for task_id, (x,x_ft,y) in enumerate(pbar):   
        ...
        # import pdb; pdb.set_trace()     #Ankit
        ## Loading optimizer
        optimizer = torch.optim.Adam([condition], lr=0.005)
        ...
        try:
            with open("ProSpect_Classifier/error_dict/error_dict_finetune_del.json", "r") as file:
                existing_data = json.load(file)
        except FileNotFoundError:
            existing_data = {"predict": [], "actual": [], "running_loss" : []}
        ...
        for i in pbar1:
            ...
            loss2=l2_loss(model,latent,args,condition,class_idx.index(label))  #Ankit
            ...
            ### Just to check if gradient is being calculated
            # if loss.grad is not None:
            #     print("Gradients are being computed for the tensor.")
            # else:
            #     print("Gradients are not being computed for the tensor.")
            ###
            ...
            current_total += 1
            if current_total > 0:
                pbar1.set_description(f'loss: {loss2 : .2f} , accuracy : {(current_correct/current_total)*100 :.2f}%')
                
            print(f"the pred idx : {class_idx[pred_idx]}, actual idx : {int(label.item())}")
            error["predict"].append(class_idx[pred_idx])
            error["actual"].append(int(label.item()))
            
        error["running_loss"].append(running_loss)
        ...
        if initial_accuracy <= (correct/total):  
            ...
            torch.save(class_embeddings,f=f"ProSpect_Classifier/logs/class_embeddings_l2+Contrast_{task_id}.pth")
            
            
        existing_data["predict"].extend(error["predict"])
        existing_data["actual"].extend(error["actual"])
        existing_data["running_loss"].append(running_loss)
            
        with open("ProSpect_Classifier/error_dict/error_dict_finetune_del.json","w") as file:
            json.dump(existing_data,file)
    
        filename = "ProSpect_Classifier/logs/accuracy_logs_del.txt"        
